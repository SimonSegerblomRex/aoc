...
def b(data):
    ...
    while True:
        ...
        relative_base = 0
        finished = False
        where_to_go = where_to_go_orig.copy()
        inp = 0
        tmp = []
        score = 0
        wall = []
                        ...
                        if 0:
                            ...
                            while inp < -1:
                                ...
                                if k == "a":
                                    ...
                                    inp = 0
                    if int(ball.imag + 1) == int(paddle.imag) and dir.imag > 0:
                        try:
                            next_goal = where_to_go.pop(0)
                        except IndexError:
                            goal = -1
                tmp = []
        if len(blocks):
            if goal == -1:
                where_to_go_orig.append(int((ball - 2*dir + 1j).real))
            print(where_to_go_orig)
    breakpoint()
